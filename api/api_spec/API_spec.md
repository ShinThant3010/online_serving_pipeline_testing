...
This mirrors the current FastAPI application version metadata.
...
If a caller sends `X-Correlation-Id`, the API echoes it back. Otherwise the server generates `corr_<uuid>`.
...
| ----- | ---- | -------: | ----- |
| studentId | string | ✅ | Non-empty student identifier used as the cache key and recommendation lookup input |
